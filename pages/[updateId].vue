...
function goToPost(post: (EnrichedPost | undefined) | Ref<EnrichedPost | undefined>) {
  ...
  const to = { name: 'updateId', params: { updateId: postValue.recordId } }
  navigateTo(to)
}
  ...
  <div class="flex flex-col gap-4 md:gap-10 justify-center items-center w-full h-full p-10">
    <div class="flex flex-col gap-4 justify-center items-center">
      ...
      <h1 class="nes-text is-warning text-center">
        Postado em {{ post.record.createdAt.toLocaleString('pt-br', {
    ...
    <div class="flex flex-row items-center gap-2">
      <button class="nes-btn" title="Primeiro Update"
        :class="!previousPost ? ['is-disabled', 'cursor-not-allowed'] : ['is-primary']" :disabled="!previousPost"
        @click="() => goToPost(firstPost)">
        {{ "<<" }}<span class="hidden md:inline"> Primeiro</span>
      </button>
      <button class="nes-btn" :class="!previousPost ? ['is-disabled', 'cursor-not-allowed'] : ['is-primary']"
        :disabled="!previousPost" @click="() => goToPost(previousPost)">
        {{ "<" }}<span class="hidden md:inline"> Anterior</span>
      </button>
      <span class="nes-text is-disabled hidden md:inline">|</span>
      <button class="nes-btn" :class="!nextPost ? ['is-disabled', 'cursor-not-allowed'] : ['is-primary']"
        :disabled="!nextPost" @click="() => goToPost(nextPost)">
        <span class="hidden md:inline">Próximo </span>{{ ">" }}
      </button>
      <button class="nes-btn" title="Último update"
        :class="!nextPost ? ['is-disabled', 'cursor-not-allowed'] : ['is-primary']" :disabled="!nextPost"
        @click="() => goToPost(lastPost)">
        <span class="hidden md:inline">Último </span>{{ ">>" }}
      </button>
    </div>
    <a href="/" class="nes-btn is-success">
