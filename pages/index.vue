  ...
  <div class="my-container flex justify-center align-middle flex-col">
    ...
    <main class="md:mx-24 flex flex-col gap-4 text-xs md:text-base">
      <div class="text-center">
        ...
        <button type="button" class="nes-btn is-warning mt-4" :class="{ 'is-disabled': !enableNotificationButton }"
      ...
      <LoadingContainer :status dark>
        ...
        <template #default>
          <div class="flex flex-col gap-4">
            <UpdateCard v-for="post of posts" :id="post.recordId" :key="post.recordId" dark rounded :post
              :title="getTitle(post)" :title-link="`/${post.recordId}`" />
          </div>
        </template>
      </LoadingContainer>
